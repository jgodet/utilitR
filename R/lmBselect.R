...
#' lmBselect(y~.,data=daten,nIter=10000,graphOutput=FALSE,effect="randomPrior")
#'
...
lmBselect <- function(formula, data, graphOutput = TRUE, nIter=10000, thin=1,effect = "random"){
  ...
  if(effect == "fixed"){
    ...
    output=coda.samples(model=modelFE,variable.names=c("alpha","beta","ind","tau"),
